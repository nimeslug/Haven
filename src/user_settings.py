...
user_settings.py
----------------
Kullanıcının kişisel tercihleri ve pet durumu kalıcı kayıt.
JSON dosyasına yazılır, uygulama yeniden açıldığında yüklenir.
"""
...
@dataclass
class PetState:
    """Bir pet'in kalıcı durumu (açlık, mutluluk vb.)."""
    hunger: float = 80.0                # 0 (aç) - 100 (tok)
    last_saved_ts: float = 0.0          # UNIX timestamp — son kayıt zamanı
    last_fed_ts: float = 0.0            # UNIX timestamp — son yem verildi


@dataclass
class UserSettings:
    """Tüm kalıcı kullanıcı ayarları."""
...
class UserSettingsStore:
    """Ayarları JSON dosyasından okuyan/yazan yönetici."""
    ...
    def _load(self) -> UserSettings:
        ...
        try:
            ...
            print(f"UYARI: user_settings.json okunamadı: {e}. Varsayılanlar kullanılacak.")
            return UserSettings()
